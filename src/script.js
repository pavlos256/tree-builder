...
let backgroundColor = 255, fontSize = 5, lineColor = 100, lineWidth = 1.;
let nodeColor = 50, nodeStrokeColor = 100, nodeStrokeWeight = 3, nodeFontColor = 255, nodeHoverColor = 90;
...
let myCanvas;
let nodeModal, nodeModalInput, nodeModalColor, nodeModalDelBtn;
let currentNode;
...
function setup() {
	...
	genButton.size(undefined, textBox.size().height);
	genButton.position(textBox.x + textBox.width + 8, textBox.position().y + (textBox.size().height - genButton.size().height) / 2);
	...
	nodeModalColor = document.getElementById('nodeModalColor');
	nodeModalDelBtn = document.getElementById('nodeModalDelBtn');
	nodeModalDelBtn.onclick = deleteCurrentNode;

	root = Node.deserialize('[1,2,3:#008000,null,7,4,5,null,null,6]');
...
function modalEditorDone() {
	...
	currentNode.color = nodeModalColor.value === "#000000" ? null : nodeModalColor.value;
	textBox.elt.value = Node.serialize(root);
...
class Node {
	constructor(val = 0., color = null, x = 0., y = 0.) {
		this.val = val;
		this.color = color;
		this.left = null;
		this.right = null;
	...
	display(parent = null) {
		...
		if (this.ellipse == null) {
			...
			this.ellipse.uxNoStrokeState = false;
			this.ellipse.uxStrokeColor = nodeStrokeColor;
			this.ellipse.uxStrokeWeight = nodeStrokeWeight;
		...
		this.ellipse.uxFill = this.color || nodeColor;
	...
	onClick() {
		...
		nodeModalColor.value = this.color || "#000000";
		currentNode = this;
		if (this === root) {
	...
	static serializeNode(node) {
		...
		return String(node.val) + (node.color == null ? "" : (":" + node.color));
	}

	static deserializeNode(s) {
		if (s) {
			s = String(s).trim();
			if (s) {
				...
				if (parts.length > 0) {
					...
					let color = parts.length > 1 ? (parts[1] || null) : null;
	...
	static deserialize(data) {
		...
		let root = Node.deserializeNode(valArr[0]);
		if (!root) {
			return null;
		}
		...
		while (valArr.length > 0) {
			...
			node = Node.deserializeNode(valArr[0]);
			if (node) {
				p.left = node;
				queue.push(p.left);
			}
